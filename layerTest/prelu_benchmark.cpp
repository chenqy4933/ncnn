...
int test_prelu(void)
{
    ...
    const char *layer_name = "PReLU";
#ifdef TEST_ACCURACY
    int circle_num = 1;
#else
    int circle_num = 1000;
#endif
    BenchLayer *bench_prelu = new BenchLayer(layer_name, circle_num);
    ///////////////////////////////////////////配置层的参数，根据各层里面定义的参数顺序来定义
    int num_slope = 1;
    float slope = 0.5f;
    bench_prelu->paramdict_->set(0, num_slope);
    //bench_prelu->paramdict_->set(0, slope);
    bench_prelu->layer_->load_param(*(bench_prelu->paramdict_)); //配置层里面的参数
    ////////////////////////////////////////////////生成数据
    ////////////输入数据
#ifdef TEST_ACCURACY
    int w = 22;
    ...
    /////////////////////权重，bias数据
    PReLU *player = dynamic_cast<PReLU *>(bench_prelu->layer_);
    player->slope_data.create(num_slope);
    float *weight = (float *)player->slope_data.data;
    ...
    /////////////////////////////////////////////进行benchmark
    Mat out_mat;
    bench_prelu->benchmark_layer(in_mat, out_mat);
    bench_prelu->print_benchmark();
    int out_w = in_mat.w;
    int out_h = in_mat.h;
    int out_c = in_mat.c;
    if (!benchmark)
